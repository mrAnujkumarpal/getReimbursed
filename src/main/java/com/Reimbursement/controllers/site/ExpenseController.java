...
public class ExpenseController extends Validate {
  ...
  @Autowired private XlsxReport xlsxReport;
  ...
  public ModelAndView createEditExpense(
    ...
    if (success.equalsIgnoreCase("false")) {
      ...
      if (expense.getVendor().getVendor_Id() == 2685) {
        ...
        // System.out.println("Vendor Location id jo jake DB m save hoge ::" +
        // ven.getLocation().getLocation_id());
        // System.out.println("dono same hone hai jo jake DB m save hoge ::" +
        // commonService.getLocationById(ven.getLocation().getLocation_id()));
      ...
      expense.setExpenseStatus(expenseService.expenseStatusDetails(1));
      ...
      if (addedExpense != null && expense.getBillable()) {
        ...
        try {
          for (MultipartFile uploadedFile : uploadingFiles) {
            ...
            // System.out.println(" DIFFERENCIATE_BILL =======================::" +
            // DIFFERENCIATE_BILL);
            // File file = new File(CREATED_DIRECTORY + DIFFERENCIATE_BILL +
  ...
  public ModelAndView viewPerticularExpense(@PathVariable("expense_Id") int expense_Id)
    ...
    if (expense != null) {
      ...
      int expenseStatusId = expense.getExpenseStatus().getExpStatus_Id();
      int loggedInEmpId = employee.getId();
      if (expense.getEmployee().getId() == loggedInEmpId
          || expense.getEmployee().getSubimitter_To().getId() == loggedInEmpId
          || expense.getEmployee().getApprover_To().getId() == loggedInEmpId
          || (employee.getEmpRole().getId() == 6
              && (expenseStatusId == 4 || expenseStatusId == 5))) {

        mv.addObject("aboutExpense", expense);

        if (expenseStatusId == 6) {
          System.out.println("this expense now is in rejected state");
          mv.addObject(
              "rejextExpDetails", expenseService.getExpenseRejectReasonData(expense.getExp_id()));
        }
      ...
      String action = expenseService.expenseStatusDetails(expenseStatusId + 1).getExpStatus_Name();
      action = action.substring(0, action.length() - 2);

      mv.addObject("employeeRoleId", employee.getEmpRole().getId());
      mv.addObject("payModeList", commonService.getAllPaymentMode());
      mv.addObject("nextAction", action);
    } else {
  ...
  public String submitParticularExpense(@PathVariable("expense_Id") int expense_Id) {
    ...
    if (ex != null) {
      ...
      Employee employee = loggedInEmployee();
      int loggedInEmployeeId = employee.getId();
      Date currentDate = new Date();
      if ((ex.getEmployee().getId() != loggedInEmployeeId)
          || (ex.getEmployee().getId() == loggedInEmployeeId && oldStatus == 1)) {
        ...
        if (oldStatus <= 5) {
          ...
          switch (newStatus) {
            case 2:
              ...
              ex.setExp_submitByEmpId(loggedInEmployeeId);
              break;
            case 3:
              ex.setExp_approvedDate(currentDate);
              ex.setExp_approveByEmpId(loggedInEmployeeId);
              break;
            case 4:
              ex.setExp_auditedDate(currentDate);
              ex.setExp_auditByEmpId(loggedInEmployeeId);
              break;
            case 5:
              ex.setExp_reimbursedDate(currentDate);
              ex.setExp_rembrsByEmpId(loggedInEmployeeId);
              break;
            default:
              break;
          }

          ExpenseStatus es = expenseService.expenseStatusDetails(newStatus);
          ex.setExpenseStatus(es);
          expenseService.saveExpense(ex);
        }
      } else {
        System.out.println(
            "Both are same and expense not in created state."
    ...
    if (ex != null) {
      ...
      if (oldStatus == 5) {
        ...
        if ((ex.getEmployee().getId() != logedInEmployeeId)
            || (ex.getEmployee().getId() == logedInEmployeeId && oldStatus == 1)) {
          ...
          if (oldStatus < 5) {
            ...
            String byName = employeeFullName(loggedInEmployee());
            System.out.println(
                empName + " This empliyee is differ from loged in emp " + logedInEmployeeId);
            /* emailService.sendHTML_ExpenseMail(
                empName, emp.getEmail(), ex.getExp_amount(),
                ex.getExp_id(), byName,
                ex.getExpenseStatus().getExpStatus_Name()
  ...
  public ModelAndView submitExpense(ModelMap model, HttpServletRequest request) {
    ...
    expenseService.submitExpenseToNextLevel(ids);
    ModelAndView mv = new ModelAndView("expense/createNew");
  ...
  @RequestMapping(
      value = "/getPDFFile/{exStatusID}/{fromDate}/{toDate}",
      method = RequestMethod.GET)
  public void downloadPDFFile(
      HttpServletResponse response,
      @PathVariable("exStatusID") String exStatusID,
      @PathVariable("fromDate") String fromDateStr,
      @PathVariable("toDate") String toDateSrt)
      throws Exception {
  ...
  @RequestMapping(
      value = "/getXLSXFile/{exStatusID}/{fromDate}/{toDate}",
      method = RequestMethod.GET)
  ...
  private List<Expense> activityBetweenDte(int ckStatusId, Date fromDate, Date toDate) {
    ...
    if (ckStatusId == 1) {
      expenseList = expenseService.expensesCreatedByMe(employee);
    } else {
        ...
        switch (ckStatusId) {
          case 1:
            if (exp.getExp_createdDate() != null) {
              if (!(exp.getExp_createdDate().after(fromDate)
                  && exp.getExp_createdDate().before(toDate))) {
          ...
          case 2:
            if (exp.getExp_submittedDate() != null) {
              if (!(exp.getExp_submittedDate().after(fromDate)
                  && exp.getExp_submittedDate().before(toDate))) {
          ...
          case 3:
            if (exp.getExp_approvedDate() != null) {
              if (!(exp.getExp_approvedDate().after(fromDate)
                  && exp.getExp_approvedDate().before(toDate))) {
          ...
          case 4:
            if (exp.getExp_auditedDate() != null) {
              if (!(exp.getExp_auditedDate().after(fromDate)
                  && exp.getExp_auditedDate().before(toDate))) {
          ...
          case 5:
            if (exp.getExp_rejectedDate() != null) {
              if (!(exp.getExp_reimbursedDate().after(fromDate)
                  && exp.getExp_reimbursedDate().before(toDate))) {
          ...
          default:
            if (exp.getExp_rejectedDate() != null) {
              if (!(exp.getExp_rejectedDate().after(fromDate)
                  && exp.getExp_rejectedDate().before(toDate))) {
                System.out.println("Removing getExp_reimbursedDate " + exp.getExp_id());
                tempExp.add(exp);
              }
            }
